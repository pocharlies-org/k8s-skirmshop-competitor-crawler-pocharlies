...
- [blocked] **Writer append-only implementado.** El crawler o módulo de ingest F3 escribe observaciones desde resultados F1/F2 sin tocar cart/probe ni grafo histórico; repetir mismo `run_id` no duplica. Evidence: `rho-backend` Claude CLI timed out `124`; no accepted backend code exists; see `backend.blocker.report.md`.
- [blocked] **Dos corridas verificadas.** En entorno de prueba o target aprobado se insertan 2 corridas con >=3 productos; cada producto tiene >=2 observaciones o el caso gap intencionado; queries demuestran delta correcto para venta, reposición/gap `indeterminate`, y ausencia de duplicados tras re-run. Evidence: blocked on backend implementation and approved DB/test target; see `backend.blocker.report.md`.
- [blocked] **Smoke de 3 productos.** Reporte con SQL/output para: producto con delta positivo estimado, producto con reposición o stock creciente `indeterminate`, producto con gap/fallo `indeterminate`. Evidence: blocked on backend implementation and DB/test target; see `backend.blocker.report.md`.
- [blocked] **Tests locales pasan.** Unit/integration tests de DDL/view/writer/idempotencia, más tests existentes de crawler; `git diff --check` PASS en repos tocados. Evidence: no backend implementation exists; `git diff --check` for RSO docs is PASS, but F3 product tests are blocked.
...
- [blocked] **rho-backend** - writer/ingest append-only, tests, fixtures de dos corridas. Evidence: Claude CLI exited `124`; see `backend.blocker.report.md`.
