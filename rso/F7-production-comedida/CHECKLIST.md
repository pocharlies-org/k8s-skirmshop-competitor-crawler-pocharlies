...
- 2026-06-27T00:45:00+02:00 - DEVOPS HARDENING PARTIAL PASS: manifests disabled endurecidos (`replicas: 0`, secret required, `REQUIRE_BRAIN_API_KEY=true`, pod/container securityContext, `/tmp` emptyDir) y CI configurado para Docker build smoke del crawler. Evidence: `kubectl kustomize k8s` PASS; `kubectl apply --dry-run=server -k k8s` PASS; `pytest -q` -> 142 passed; `python3 -m compileall src tests` PASS; YAML parse OK. F7 sigue bloqueada por image tag/digest, `competitor_intel` live, Argo app disabled, prober transport/egress, CronJob y noche real.
