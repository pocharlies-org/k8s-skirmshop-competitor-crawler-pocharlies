...
- [ ] Imagenes siguen `:pending`, pero crawler ya tiene CI build smoke PASS y release workflow preparado. Evidence: GitHub Actions CI run `28291764829` PASS (`Build images` 1m16s, `Lint and validate manifests` 41s); `.github/workflows/release.yml`; `k8s/manifest.yaml` y `k8s/prober-deployment.yaml` siguen `:pending`.
